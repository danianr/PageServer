   ...
   <xsl:when test="/ninja/purchaseGateway">
	...
	<table summary="select page value and value in account" border="0" align="center" cellpadding="3" cellspacing="0" style="width: 50em">
            ...
            <tr><th colspan="2" align="left">
				...
				<xsl:if test="purchaseGateway/interactiveToken">
					<xsl:variable name="keyName"><xsl:value-of select="purchaseGateway/interactiveToken/@name"/></xsl:variable>
					<xsl:variable name="keyValue"><xsl:value-of select="purchaseGateway/interactiveToken/@value"/></xsl:variable>

                    
                    <form method="get" style="margin: 15px 2px; padding: 5px" action="{$targetURL}">
